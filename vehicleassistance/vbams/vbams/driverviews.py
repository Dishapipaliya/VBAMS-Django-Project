...
from vbamsapp.models import CustomUser,Driver,Booking,Tracking,Vehicle
...
def VEHICLE_PROFILE(request):
    user = get_object_or_404(CustomUser, id=request.user.id)  # Fetch the current user
    vehicle = Vehicle.objects.filter(owner=request.user.id).first()  # Adjust field name as needed
    context = {
        "user": user,
        "vehicle": vehicle,  # Replace 'driver' with 'vehicle'
    }
    return render(request, 'vehicle/vehicle-profile.html', context)  # Update the template path
